...
class AccessTokenTest(BaseOAuth2TestCase):
    ...
    def test_access_token_additional_data_handler(self):
        """
        Verify that the access token view gets data via the additional data handler.
        """
        with self.settings(OAUTH2_ADDITIONAL_DATA_HANDLER=None):
            original_response = self._login_authorize_get_token()

        with self.settings(OAUTH2_ADDITIONAL_DATA_HANDLER='oauth2_provider.tests.utils.fake_handler'):
            handled_response = self._login_authorize_get_token()

        # Verify the responses differ
        self.assertNotEqual(original_response, handled_response)

        # Verify the additional data is included in the response
        self.assertTrue(all(item in handled_response.items() for item in HANDLER_DATA.items()))
